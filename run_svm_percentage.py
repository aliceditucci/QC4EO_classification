...
def main(): 
    ...
    with open(datafile, 'rb') as f:
        data = pickle.load(f)
    independent_entries = data['kernel_entries']
    gram_mtraix_train = reconstruct_gram_matrix(independent_entries)

    #make data dir
    dir = '../../../Data/aditucci/Output_percentage_data/{}/{}/N_{}/'.format(compress_method, kernel_type,N_FEATURES)
    ...
    if kernel_type == 'fidelity':
        ...
        independent_entries, score, confusion = fidelity_kernels_batch(train_features, train_labels, val_features, val_labels, ZZ_reps, ent_type, method, device)
